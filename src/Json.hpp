    ...
    namespace
    {
        ...
        JSON parse_object(const string &str, size_t &offset)
        {
            ...
            while (true)
            {
                ...
                if (str[offset] != ':')
                {
                    std::string s = "Error: Object: Expected colon, found '";
                    s += str[offset];
                    s += "'";
                    MsAssert(false, s.c_str());
                    break;
                }
                consume_ws(str, ++offset);
                ...
                {
                    std::string s = "ERROR: Object: Expected comma, found '";
                    s += str[offset];
                    s += "'";
                    MsAssert(false, s.c_str());
                    break;
        ...
        JSON parse_array(const string &str, size_t &offset)
        {
            ...
            while (true)
            {
                ...
                {
                    std::string s = "ERROR: Array: Expected ',' or ']', found '";
                    s += str[offset];
                    s += "'";
                    MsAssert(false, s.c_str());
                    return std::move(JSON::Make(JSON::Class::Array));
                    ...
                    switch (str[++offset])
                    {
                        ...
                        case 'u':
                        {
                            ...
                            for (unsigned i = 1; i <= 4; ++i)
                            {
                                ...
                                {
                                    std::string s = "ERROR: String: Expected hex character in unicode escape, found '";
                                    s += c;
                                    s += "'";
                                    MsAssert(false, s.c_str());
        ...
        JSON parse_number(const string &str, size_t &offset)
        {
            ...
            if (c == 'E' || c == 'e')
            {
                ...
                while (true)
                {
                    ...
                    {
                        std::string s = "ERROR: Number: Expected a number for exponent, found '";
                        s += c;
                        s += "'";
                        MsAssert(false, s.c_str());
            ...
            {
                std::string s = "ERROR: Number: unexpected character '";
                s += c;
                s += "'";
                MsAssert(false, s.c_str());
                return std::move(JSON::Make(JSON::Class::Null));
        ...
        JSON parse_bool(const string &str, size_t &offset)
        {
            ...
            {
                std::string s = "ERROR: Bool: Expected 'true' or 'false', found '";
                s += str.substr(offset, 5);
                s += "'";
                MsAssert(false, s.c_str());
                return std::move(JSON::Make(JSON::Class::Null));
            }
        ...
        JSON parse_null(const string &str, size_t &offset)
        {
            ...
            {
                std::string s = "ERROR: Null: Expected 'null', found '";
                s += str.substr(offset, 4);
                s += "'";
                MsAssert(false, s.c_str());
                return std::move(JSON::Make(JSON::Class::Null));
            }
        ...
        JSON parse_next(const string &str, size_t &offset)
        {
            ...
            std::string s = "ERROR: Parse: Unknown starting character '";
            s += value;
            s += "'";
            MsAssert(false, s.c_str());
            return JSON();
